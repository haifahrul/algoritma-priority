 ...
 */
class DualList extends InputWidget
{
    ...
    public function run()
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if (!in_array($value[$this->data_id], $selected)) {
                $ret .= '<option value="' . $value[$this->data_id] . '">' . " (".$value[$this->data_id].") ".$value[$this->data_value]  . '</option>' . "\n";
        } else {
                $cnt++;
                $ret_sel .= '$("#dlb-'.$this->attribute.' .selected").
                append("<option value=' . $value[$this->data_id] . '>' . " (".$value[$this->data_id].") ".$value[$this->data_value]. '</option>");';
            }

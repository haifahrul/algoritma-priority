...
class ExcelView extends \dzas\widgets\GridView {
    ...
    public $filename = ""; //Nama file eksport
...
#            spl_autoload_register(array('YiiBase', 'autoload'));
    ...
    public function renderHeader() {
        ...
        if ($this->fullExportConfig[$this->fullExportType]['header']) {
            //generate header excel
            /*
            $cell = $this->objPHPExcel->getActiveSheet()->setCellValue($this->columnName(1) . $this->tableStartRow, 'Laporan ', true);
            $this->tableStartRow++;
        ...
        // parameter insert catatan sp
         $spnote = "Jika dalam batas waktu 7 (tujuh) hari kalender terhitung dari surat ini diterbitkan Bapak/Ibu belum melakukan pembayaran secara penuh, maka
         kami akan menggunakan hak kamu untuk menguasai dengan segera kendaraan saudara yang telah kami biayai tersebut sesuai dengan Perjanjian Pembiayaan yang telah disepakati.";
         $this->objPHPExcel->getActiveSheet()->setCellValue('BZ1', $spnote, true);
        ...
        foreach ($this->columns as $column) {
            ...
            if ($column->header === null) {
                if ($provider instanceof ActiveDataProvider && $provider->query instanceof ActiveQueryInterface) {
                    /* @var $model Model */
                    //mendapatkan model dataprovider
                    $model = new $provider->query->modelClass;
                    //nilai header dalam database
        ...
        for ($i = $this->headerStartRow; $i < ($this->tableStartRow-1); $i++) {
            $this->objPHPExcel->getActiveSheet()->mergeCells($this->columnName(1) . $i . ":" . $this->columnName($this->tableEndColumn  + 4) . $i);
            if ($this->fullExportType == self::FULL_EXCELX)
    ...
    public function renderBody() {
        //Mendapatkan model dri dataprovider
        ...
        foreach ($models as $index => $model) {
            $key = $keys[$index];
            $end = $keys;
            //menulis data per row
            $rows[] = $this->renderRow($model, $key, $index, $end);
        }
    ...
    public function renderRow($model, $key, $index, $end) {
        $cells = [];
        /* @var $column Column */
        $this->tableEndColumn = 0;
        foreach ($this->columns as $column) {
        $endColumn =count($this->columns);
            if($column instanceof \yii\grid\SerialColumn )
           ...
           elseif ($column->attribute !== null) {
            ...
            $this->tableEndColumn++;
            //increament untuk menambahkan tiap kolumn
            $currentRow = $index + $this->tableStartRow + 1;
       
            $cell = $this->objPHPExcel->getActiveSheet()->setCellValue($this->columnName($this->tableEndColumn) . ($currentRow),
    ...
    public function renderFooter($row) {
        ...
        foreach ($this->columns as $n => $column) {
            ...
            if ($column->footer) {
                ...
                $cell = $this->objPHPExcel->getActiveSheet()->setCellValue($this->columnName($this->tableEndColumn) . ($row + 2), $footer."zzafdafafaazaa", true);
    ...
    public function run() {
        if ($this->grid_mode == 'export') {
            ...
            //tambahan yang tidak di dalam database
            $this->objPHPExcel->getActiveSheet()->getColumnDimension($this->columnName($endColumn+1))->setAutoSize(true);
            $this->objPHPExcel->getActiveSheet()->getColumnDimension($this->columnName($endColumn+1))->setAutoSize(true);

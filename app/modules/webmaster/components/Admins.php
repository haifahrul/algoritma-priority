...
namespace app\modules\webmaster\components;
...
Class Admins extends \yii\base\Object
{
   ...
   * ];
   * $items = Administrator::filterRouteMenu($items);
   * if(count($items)>0){
  ...
  public static function filterRoute($route,$strict=false)
  {
      ...
      if(!$strict){
          ...
          $authItems = AuthItem::find()->select('name')->where(['like','name',$parent])->asArray()->all();
